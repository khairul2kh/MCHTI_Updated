...
public class HibernateBillingDAO implements BillingDAO {
	...
	public List<PatientSearch> searchListOfPatient(Date date, String searchKey,
		...
		String endDate = sdf.format(date) + " 23:59:59";
                ...
                String ipdBill="IPD ADMISSION BILLING";
                String bill="BILLING CLERK";
                User user = Context.getAuthenticatedUser();
                String hql=null;
                if(user.hasRole(ipdBill, true)){
                     hql = "from PatientSearch ps where ps.patientId IN (SELECT o.patient FROM OpdTestOrder o where o.createdOn BETWEEN '"
				+ startDate
				+ "' AND '"
				+ endDate
				+ "' AND o.billingStatus=0 AND o.typeConcept=5 AND o.cancelStatus=0 AND o.billableService is NOT NULL GROUP BY o.patient) AND (ps.identifier LIKE '%"
				+ searchKey + "%' OR ps.fullname LIKE '" + searchKey + "%')"; }
                
                else if(user.hasRole(bill, true)) {
                
                    hql = "from PatientSearch ps where ps.patientId IN (SELECT o.patient FROM OpdTestOrder o where o.createdOn BETWEEN '"
				+ startDate
				+ "' AND '"
				+ endDate
				+ "' AND o.billingStatus=0 AND o.typeConcept<=4 AND o.cancelStatus=0 AND o.billableService is NOT NULL GROUP BY o.patient) AND (ps.identifier LIKE '%"
				+ searchKey + "%' OR ps.fullname LIKE '" + searchKey + "%')";
                }
                
                else if(user.hasRole(HospitalCoreConstants.ROLL_SYSTEM_DEVELOPER)) {
