...
public class HibernateIpdDAO implements IpdDAO {
    ...
    public IpdPatientIntake saveIpdPatientIntake(IpdPatientIntake ipdPatientIntake)
            throws DAOException {
        return (IpdPatientIntake) sessionFactory.getCurrentSession().merge(ipdPatientIntake);
    }

    public List<IpdPatientIntake> getIpdPatientIntake(
            Integer patientId, Integer patientAdmissionLogId)
            throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
                IpdPatientIntake.class);
        criteria.add(Restrictions.eq("patient.personId", patientId));
        criteria.add(Restrictions.eq("ipdPatientAdmissionLog.id", patientAdmissionLogId));
        return criteria.list();
    }

    public List<OpdDrugOrder> getOpdDrugOrderByPatientEncounterId(
            Integer patientId) throws DAOException {
    ...
    public IpdPatientAdmission getAdmissionByPatientId(Integer patientId)
            throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(IpdPatientAdmission.class);
        criteria.add(Restrictions.eq("patient.id", patientId));
        List<IpdPatientAdmission> list = criteria.list();
    ...
    public IpdPatientAdmittedQueueLog saveIpdPatientAdmittedQueueLog(IpdPatientAdmittedQueueLog admittedqueuelog) throws DAOException {
        return (IpdPatientAdmittedQueueLog) sessionFactory.getCurrentSession().merge(admittedqueuelog);
    }

    public IpdPatientAdmittedQueueLog getIpdpatientAdmittedQueueLogNew(Integer encounterId) throws DAOException {

        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(IpdPatientAdmittedQueueLog.class);
        criteria.add(Restrictions.eq("ipdEncounter.encounterId", encounterId));
        return (IpdPatientAdmittedQueueLog) criteria.uniqueResult();

    }

    public void removeIpdPatientAdmittedQueueLogNew(IpdPatientAdmittedQueueLog admittedqueuelog) {
    sessionFactory.getCurrentSession().delete(admittedqueuelog);
    }
