...
DASHBOARD={
		...
		{
			if(SESSION.checkSession()){
				url = "detailClinical.htm?id="+id+"&keepThis=false&TB_iframe=true&height=500&width=800";
				tb_show(" ",url,false);
			}
		},
		...
		onChangeDiagnosis : function(container, id, name)
		{
			if(container == 'diagnosis'){
				...
				//New code Enter here for diagnosis popup disabled 	
			...
			if(container == 'procedure'){
				...
				// New Code enter for procedure popup diasabled	
		...
		if(container == 'investigation'){
				...
				// New code Enter here for investigation popup disabled	
