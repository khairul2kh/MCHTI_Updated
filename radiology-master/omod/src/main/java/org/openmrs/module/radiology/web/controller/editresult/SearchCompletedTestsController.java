...
@Controller("RadiologySearchEditResultController")
@RequestMapping("/module/radiology/searchCompletedTests.form")
public class SearchCompletedTestsController {
	...
	public String searchTest(
		...
		try {
			...
			if (currentPage == null)
				currentPage = 1;
			List<RadiologyTest> radiologyTests = rs.getCompletedRadiologyTests(date, phrase, allowableTests, currentPage);
			...
			PagingUtil pagingUtil = new PagingUtil(GlobalPropertyUtil.getInteger(RadiologyConstants.PROPERTY_PAGESIZE, 20), currentPage,
					total);
                        ...
                        for (TestModel testModel : tests) {
                            System.out.println("test*********************88888"+testModel.getTestName());
                    }
